...
-- Policy 3: Allow users to view their own media passes (essential data only, no sensitive payment info)
CREATE POLICY "Users can view their own media passes"
ON public.media_passes
FOR SELECT
USING (auth.uid() = user_id);

-- Policy 4: Allow admins and moderators to view media passes for management
