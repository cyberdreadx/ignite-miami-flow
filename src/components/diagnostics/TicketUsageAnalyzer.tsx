...
export const TicketUsageAnalyzer: React.FC = () => {
  ...
  const analyzeSuspiciousUsage = async () => {
    ...
    try {
      ...
      tickets?.forEach(ticket => {
        ...
        const suspicious = 
          timeBetween < 10 || // Used within 10 minutes
          ticket.used_by?.includes('Test') ||
          ticket.used_by?.includes('System') ||
          (timeBetween < 60 && !ticket.used_by?.includes('Door')); // Used within 1 hour by non-door staff
  ...
  const resetTicketUsage = async (ticketId: string) => {
    ...
    try {
      ...
      setSuspiciousTickets(prev => 
        prev.map(ticket => 
          ticket.id === ticketId 
            ? { ...ticket, usedAt: '', usedBy: '', suspicious: false }
            : ticket
        )
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {suspiciousTickets.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid gap-4">
            {suspiciousTickets.map((ticket, index) => (
              <Card 
                key={ticket.id} 
                className={ticket.suspicious ? "border-red-200 bg-red-50" : "border-gray-200"}
              >
                    ...
                    <div className="flex gap-2">
                      ...
                      {ticket.suspicious && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => resetTicketUsage(ticket.id)}
                          disabled={fixing === ticket.id}
      ...
      {suspiciousTickets.length === 0 && !loading && (
        <Card>
